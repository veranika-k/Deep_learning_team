...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.exists(mask_path):
            ...
            alt_path = os.path.join(self.mask_dir, f"{base}_label{alt_ext}")
            if os.path.exists(alt_path):
                mask_path = alt_path
            else:
                raise FileNotFoundError(f"Mask for {fname} not found")
...
if __name__ == '__main__':
    ...
    model = SegmentationModel('efficientnet_b0', num_classes=3, input_channels=1).to(device)
    model.unfreeze_backbone(n_layers=len(list(model.backbone.model.children())))
    opt = optim.Adam(model.parameters(), lr=lr)
    # replaced cross-entropy with dice loss
    ...
    for e in range(epochs):
        model.train()
        run_l = 0
        for imgs, msk, _ in train_loader:
        ...
        tr_l = run_l / len(train_loader.dataset)
        train_losses.append(tr_l)

        # validation
        model.eval()
        rv = 0
        tp = fp = fn = tot_true = tot_pred = 0
        with torch.no_grad():
            for imgs, msk, _ in val_loader:
                imgs, msk = imgs.to(device), msk.to(device)
                out = model(imgs)
                out = F.interpolate(out, size=msk.shape[1:], mode='bilinear', align_corners=False)
                loss = dice_loss(out, msk)
                rv += loss.item() * imgs.size(0)
        ...
        va_l = rv / len(val_loader.dataset)
        ...
        print(f"Epoch {e+1}/{epochs} — Train Dice Loss: {tr_l:.4f} — Val Dice Loss: {va_l:.4f}"
              f" — Prec: {prec:.4f} — Rec: {rec:.4f} — F1: {f1:.4f}")

    save_predictions(model, val_loader, device, save_dir='val_preds_new_dice')
    torch.save(model.state_dict(), os.path.join(wd, 'weights_final_new_dice.pth'))
    print("Saved val preds and final weights.")

    plt.figure()
    epochs_range = range(1, epochs+1)
    plt.plot(epochs_range, train_losses, marker='o', label='Train Dice Loss')
    plt.plot(epochs_range, val_losses,   marker='o', label='Val Dice Loss')
    plt.plot(epochs_range, val_f1,       marker='o', label='Val F1')
    plt.title('Metrics over Epochs')
    plt.xlabel('Epoch'); plt.ylabel('Value')
    plt.grid(True); plt.legend()
    plt.savefig('loss_plot_new_dice.png')
    print("Saved metric plot to loss_plot_new_dice.png")
